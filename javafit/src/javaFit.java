...
/**
 * @desc Project Phase 1. Do not change this file. Your code should work
 *       flawlessly with this file when completed.
 * @author Glen
 *
 */
...
public class javaFit {
	...
	public static void main(String[] args) {
		...
		// NOTE: You do not need to handle cases where the user makes a typo. It is okay
		// to error on that code, because we will not, in the future, allow direct user
		// input like this.
		...
		// Print out the full information on workouts that use a specific equipment and
		// work a muscle (primary or secondary)
		...
		Workouts chosenWorkouts = workouts.getWorkoutsByMuscle(Workouts.Muscle.valueOf(muscleAgain), true)
				.getWorkoutsByEquipment(Workouts.Equipment.valueOf(equipment));
		...
		ArrayList<String[]> refinedWorkoutData = workouts
				.getWorkoutsByMuscle(Workouts.Muscle.valueOf(muscleAgain), true).getWorkoutsByEquipment(equipmentList)
				.getFullInformation(); // Take note of the valueOf() method. You will need this for your code.
